...
export const getQuestionAndCorrectAnswer = () => {
    const initialTerm = Math.floor(Math.random() * (maxInitialTermNumber - minInitialTermNumber + 1)) + minInitialTermNumber;
    const commonDifference = Math.floor(Math.random() * (maxCommonDifferenceNumber - minCommonDifferenceNumber + 1)) + minCommonDifferenceNumber;
    const progression = [];
    let i = 0;
